...
def shop_page(request):
    """Сторінка магазину з товарами та кількістю в корзині"""
...
def contact_page(request):
    if request.method == "POST":
        ...
        info = (
            "Чудово! Менеджер невдовзі з вами зв*яжеться."
            if success else
            "Сталася помилка при відправці. Спробуйте пізніше."
        )
...
def create_order(request):
    try:
        ...
        message_lines.append(f"💰 ЗАГАЛЬНА СУМА: {total_price} грн")
...
def cart_update(request, product_id):
    ...
    cart.add(product_id, new_quantity - current_quantity)
    return redirect('cart_detail')
